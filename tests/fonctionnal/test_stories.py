...
@pytest.mark.usefixtures("driver_edge_init")
class TestStories_5_to_9:
    """
    For each test, the system open an edge browser
    and connect to the 127.0.0.1:500 page
    """

    def enter_email(self, club_name):
        field_email = self.driver.find_element(By.ID, "email_field")
        field_email.send_keys(club_name + "@exemple.com")
        field_email.submit()
    ...
    def test_story5_bookinfo(self):
        self.enter_email('club')
        self.click_on_book_link()
        field_places = self.driver.find_element(By.ID, "number_of_places")
        field_places.send_keys("1")
        time.sleep(2)
        field_places.submit()
    ...
    def test_story7_booking_more_available(self):
        ...
        time.sleep(2)
        field_places.submit()
        assert "Your club doesn't have enough points"\
            in self.driver.page_source
        time.sleep(2)
        self.click_on_logout()
        self.enter_email('club3')
        self.click_on_book_link()
        field_places = self.driver.find_element(By.ID, "number_of_places")
    ...
    def test_story8_booking_more_12(self):
        ...
        """
        self.enter_email('club2')
        # try to book 20 places
        self.click_on_book_link()
        time.sleep(4)
        field_places = self.driver.find_element(By.ID, "number_of_places")
        field_places.send_keys("13")
        time.sleep(2)
        field_places.submit()
        print(self.driver.page_source)
        assert "You cannot purchase more than 12 places"\
            in self.driver.page_source
        # try to book 10 + 3 places
        field_places = self.driver.find_element(By.ID, "number_of_places")
